...
summarise(mean = mean(quality), sd = sd(quality), n = n())

diff_mean <- diff(df_stats$mean)

sp_squared <- sum((df_stats$n - 1) * df_stats$sd^2) / sum(df_stats$n - 2)
sp <- sqrt(sp_squared * (1 / df_stats$n[1] + 1 / df_stats$n[2]))
...
The t statistics t1, t2 and t3 are closely matched and notably high, signifying a consistent and significant distinction in quality between the two wines.
...
```
The simple models highlight the significance of citric_acid and total_sulfure dioxide in influencing wine quality. 
...
vif(model)
```
VIF values reveal whether predictors are too correlated which affects the model's accuracy.
...
Stepwise selection manually picks variables, LASSO eliminates unimportant ones by setting their coefficients to zero, and ridge reduces overfitting by shrinking coefficients without excluding any variables.
...
Including all subsets of 10 predictors, we can create 1,024 different models, ranging from using no predictors to using all. 
...
* Are `aic_formula` and `rsq_formula` the same? How do they differ from the formulas shortlisted in question 3? The aic_formula and rsq_formula are not identical; they typically feature fewer predictors than those from question 3, focusing on model efficiency and explanatory power.


* Which of these is more reliable? Why? AIC is deemed more reliable as it assesses model performance with consideration for complexity, unlike rsq, which only measures fit to the training data without penalizing for overfitting.

* If we had a dataset with $10,000$ columns, which of these methods would you consider for your analyses? Why? For a dataset with 10,000 columns, LASSO or ridge regression is favored, as these methods efficiently manage large-scale variable selection, reducing the model to only significant predictors.
